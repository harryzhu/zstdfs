...
func StartHTTPServer() {
	...
	tmpl.Delims("{{", "}}")
	tmpl.AddFunc("NowToDate", NowToDate)
	tmpl.Reload(IsDebug)
	...
	assetAPI := app.Party("/assets")
	{
		assetAPI.Use(iris.Compression)
		assetAPI.HandleDir("/", iris.Dir(AssetDir), iris.DirOptions{ShowList: false})
	}
	...
	userFileAPI := app.Party("/f/")
	{
		...
		userFileAPI.Head("/{uname:string}/{key:path}", headFiles)
	}
...
func appShutdown() {
	ctx, cancel := stdContext.WithTimeout(stdContext.Background(), time.Second*3)
	defer cancel()
	// close all hosts
...
func headFiles(ctx iris.Context) {
	uname := ctx.Params().Get("uname")
	key := ctx.Params().Get("key")
	DebugInfo("headFiles:key", key)
	...
	meta := mongoGetByKey(uname, "_id", key)
...
func topTags(ctx iris.Context) {
	...
	viewData["min_top_caption"] = MinTopCaption
