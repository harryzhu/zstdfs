...
func (vs *volumeService) GetSyncFileList(ctx context.Context, FileIn *pbv.SyncFileList) (*pbv.SyncFileList, error) {
	var size int
	var key string
	err := DBMETA.QueryRow("SELECT key,size FROM data where key=?", "db-meta-is-ok").Scan(&key, &size)
	if err != nil {
		Logger.Error("cannot GetSyncFileList")
	}

	return &pbv.SyncFileList{}, nil
}
